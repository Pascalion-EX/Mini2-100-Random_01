...
class MiniApplicationTests {
	...
	// --- TRIP ---
	@Test
	public void testControllerFindTripsByCaptainIdWithResults() {
		Captain captain = captainService.addCaptain(new Captain("TripCaptain", "LIC999", 4.1));
		Customer customer = customerService.addCustomer(new Customer("TripCustomer", "tc@domain.com", "9998887777"));

		Trip trip = new Trip(LocalDateTime.now(), "Start", "End", 90.0);
		trip.setCaptain(captain);
		trip.setCustomer(customer);
		tripService.addTrip(trip);

		ResponseEntity<List> response = restTemplate.getForEntity(
				BASE_URL_TRIP + "/findByCaptainId?captainId=" + captain.getId(), List.class);
		assertEquals(HttpStatus.OK, response.getStatusCode());
		assertNotNull(response.getBody());
	...
	@Test
	public void testControllerAddTripWithRelations() {
		Captain captain = captainService.addCaptain(new Captain("Linked Captain", "LIC555", 4.5));
		Customer customer = customerService.addCustomer(new Customer("Linked Customer", "link@domain.com", "1231231231"));

		Trip trip = new Trip(LocalDateTime.now(), "Downtown", "Airport", 75.0);
		trip.setCaptain(captain);
		trip.setCustomer(customer);
		...
		assertEquals(HttpStatus.OK, response.getStatusCode());
		assertNotNull(response.getBody());
		assertEquals("Downtown", response.getBody().getOrigin());
	}

	@Test
	public void testControllerUpdateTripInvalidId() {
		Trip updatedTrip = new Trip(LocalDateTime.now(), "Nowhere", "Somewhere", 99.0);
		HttpHeaders headers = new HttpHeaders();
		headers.setContentType(MediaType.APPLICATION_JSON);
		HttpEntity<Trip> request = new HttpEntity<>(updatedTrip, headers);

		ResponseEntity<Trip> response = restTemplate.exchange(
				BASE_URL_TRIP + "/update/99999",
				HttpMethod.PUT,
				request,
				Trip.class
		);
	...
	// --- PAYMENT ---
	@Test
	public void testControllerFindPaymentsByTripId() {
		Captain captain = captainService.addCaptain(new Captain("PaidCaptain", "PCAP123", 4.7));
		Customer customer = customerService.addCustomer(new Customer("PaidCustomer", "paid@domain.com", "1233211234"));

		Trip trip = new Trip(LocalDateTime.now(), "StartPt", "EndPt", 130.0);
		trip.setCaptain(captain);
		trip.setCustomer(customer);
		tripService.addTrip(trip);

		Payment payment = new Payment(130.0, "Cash", true);
		payment.setTrip(trip);
		paymentService.addPayment(payment);

		ResponseEntity<List> response = restTemplate.getForEntity(
				BASE_URL_PAYMENT + "/findByTripId?tripId=" + trip.getId(), List.class);
		assertEquals(HttpStatus.OK, response.getStatusCode());
		assertNotNull(response.getBody());
		assertFalse(response.getBody().isEmpty());
	}
	...
	// --- RATING (MongoDB) ---
	@Test
	public void testControllerAddRatingInvalidEntityType() {
		Rating invalidRating = new Rating(1L, "invalid_type", 3, "Bad input.", LocalDateTime.now());
		HttpHeaders headers = new HttpHeaders();
		headers.setContentType(MediaType.APPLICATION_JSON);
		HttpEntity<Rating> request = new HttpEntity<>(invalidRating, headers);

		ResponseEntity<String> response = restTemplate.postForEntity(BASE_URL_RATING + "/addRating", request, String.class);
		// Your controller should validate the entityType and return error or ignore
		assertTrue(response.getStatusCode().is2xxSuccessful() || response.getStatusCode() == HttpStatus.BAD_REQUEST);
	}

	@Test
	public void testControllerUpdateNonExistingRating() {
		Rating updated = new Rating(10L, "trip", 5, "Update fail test", LocalDateTime.now());
		HttpHeaders headers = new HttpHeaders();
		headers.setContentType(MediaType.APPLICATION_JSON);
		HttpEntity<Rating> request = new HttpEntity<>(updated, headers);

		ResponseEntity<Rating> response = restTemplate.exchange(
				BASE_URL_RATING + "/update/0000000000",
				HttpMethod.PUT,
				request,
				Rating.class
		);
